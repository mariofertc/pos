...
class Sale_lib
{
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$tax=null,$description=null,$serialnumber=null,$almacen = null)
	{
		...
		array(
			...
			'price'=>$price!=null ? $price: round($item->unit_price+(($almacen->utilidad/100)*$item->unit_price),config('precision'))
	...
	function out_of_stock($item_id,$almacen)
	{
        ...
        if($almacenes != null){
        	$cantidad = $almacenes->cantidad;
        }
                //var_dump($almacenes->cantidad);
		if ($cantidad - $quanity_added < 0)
		{
			return true;
		}
